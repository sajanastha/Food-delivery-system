...
public class MenuItemDAO {
    ...
    public void addItem(MenuItem item) throws SQLException {
        String sql = "INSERT INTO menu_items " +
                "(restaurantID,name,description,price,category,isAvailable)" +
                " VALUES (?,?,?,?,?,?)";
    ...
    public List<MenuItem> getAvailable(int restaurantID)
        ...
        String sql = "SELECT * FROM menu_items WHERE " +
                "restaurantID=? AND isAvailable=1 ORDER BY category,name";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public List<MenuItem> getByRestaurant(int restaurantID)
        ...
        List<MenuItem> list = new ArrayList<>();
        String sql = "SELECT * FROM menu_items WHERE " +
                "restaurantID=? ORDER BY category,name";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, restaurantID);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) list.add(map(rs));
    ...
    public void updateItem(MenuItem item) throws SQLException {
        String sql = "UPDATE menu_items SET name=?,description=?," +
                "price=?,category=?,isAvailable=? WHERE itemID=?";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public void deleteItem(int itemID) throws SQLException {
        try (PreparedStatement ps = conn.prepareStatement(
                "DELETE FROM menu_items WHERE itemID=?")) {
            ps.setInt(1, itemID);
            ps.executeUpdate();
        }
    }
    ...
    public void toggleAvailability(int itemID, boolean available)
            throws SQLException {
        try (PreparedStatement ps = conn.prepareStatement(
                "UPDATE menu_items SET isAvailable=? WHERE itemID=?")) {
    ...
    private MenuItem map(ResultSet rs) throws SQLException {
        MenuItem m = new MenuItem(
            rs.getInt("itemID"), rs.getInt("restaurantID"),
